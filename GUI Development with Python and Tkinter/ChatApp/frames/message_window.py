...
class Messageroot(tk.Canvas):
    def __init__(self, container, *args, **kwargs):
        ...
        self.scrollable_root = self.create_root((0, 0), root=self.message_frame, anchor="nw")

        def configure_scroll_region(event):
            self.configure(scrollregion=self.bbox("all"))

        def configure_root_size(event):
            self.itemconfig(self.scrollable_root, width=self.winfo_width())

        self.bind("<Configure>", configure_root_size)
    ...
    def _create_message_bubble(self, container, message_content, message_time, message_labels):
        avatar_image = Image.open("./assets/th.png")
