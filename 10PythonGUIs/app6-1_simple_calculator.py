...
def submit_number(number):
    ...
    # Insert the number or decimal to the end of the display
...
def operate(operator):
    """Sotre the first number of the expression and the operation to be used"""
    global first_number
    global operation

    operation_sign = {'add':'+', 'subtract':'-', 'multiply':'*', 'divide':'/', 'exponent':'^n', 'inverse':'1/x', 'square':'^2'}
    # Get the operator pressed and the current value of the display. This is the first number in the calculation.
    operation = operator
    first_number = display.get()
    display.insert(END, operation_sign[operation])
...
def equal():
    ...
    # Perform the desired mathematics
    if operation == 'add':
        value = float(first_number) + float(final_value)
    elif operation == 'subtract':
        value = float(first_number) - float(final_value)
    elif operation == 'multiply':
        value = float(first_number) * float(final_value)
...
def square():
    global value
    """Calculate the squre of a given number"""
...
# Third row (Add padding to create the size of the colummns)
